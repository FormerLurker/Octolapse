...
import octolapse.log as log
logging_configurator = log.LoggingConfigurator(__name__)
root_logger = logging_configurator.get_root_logger()
# so that we can
logger = logging_configurator.get_logger("octolapse.__init__")
...
import octolapse.settings_migration as settings_migration
import octoprint.filemanager
import octolapse.stabilization_preprocessing
import octolapse.camera as camera
import octolapse.render as render
import octolapse.snapshot as snapshot
from octolapse.position import Position
...
from octolapse.gcode_parser import Commands
from octolapse.render import TimelapseRenderJob, RenderingCallbackArgs
from octolapse.settings import OctolapseSettings, PrinterProfile, StabilizationProfile, CameraProfile, \
    ...
    SettingsJsonEncoder
from octolapse.timelapse import Timelapse, TimelapseState
from octolapse.stabilization_preprocessing import StabilizationPreprocessingThread
import threading
import Queue
...
class OctolapsePlugin(octoprint.plugin.SettingsPlugin,
    ...
    def upload_watermark(self):
        ...
        logger.debug("Receiving uploaded watermark {0}.",image_filename)
        ...
        if not os.path.exists(full_watermarks_dir):
            logger.info("Creating watermarks directory at {}.".format(full_watermarks_dir))
        ...
        if os.path.exists(watermark_destination_path):
            if os.path.isdir(watermark_destination_path):
                logger.error(
                    "Tried to upload watermark to {} but already contains a directory! Aborting!",
                ...
                logger.warning(
                    "Tried to upload watermark to {} but file already exists! Overwriting...",
                    watermark_destination_path
                )
                os.remove(watermark_destination_path)
        logger.info(
            "Moving watermark from {0} to {1}.",
            watermark_temp_path,
            watermark_destination_path
        )
    ...
    def delete_watermark(self):
        ...
        logger.debug("Deleting watermark {0}.", filepath)
        if not os.path.exists(filepath):
            logger.error("Tried to delete watermark at {0} but file doesn't exists!", filepath)
        ...
        if not is_subdirectory(watermarks_directory, filepath):
            logger.error(
                "Tried to delete watermark at {} but file doesn't exists!",
                filepath
            )
        ...
        if not os.path.isfile(self.get_settings_file_path()) or force_defaults:
            ...
            with open(self.get_settings_file_path()) as settingsJson:
                ...
                if self._octolapse_settings is None:
                    ...
                    # be correct.
                    self.configure_loggers()
                    logger.info("Settings loaded from {0}.", self.get_settings_file_path())
                else:
    ...
    def copy_octoprint_default_settings(self, apply_to_current_profile=False):
        try:
            ...
            try:
                ...
                logger.info("Setting octolapse camera address to {0}.", camera_address)
                ...
                logger.info("Setting octolapse camera snapshot template to {0}.", snapshot_request_template)
    ...
    def on_event(self, event, payload):
        ...
        try:
            ...
            logger.debug("Printer event received:{0}.", event)
            ...
            if event == Events.CLIENT_OPENED:
                ...
                logger.info("homing to payload:{0}.".format(payload))
