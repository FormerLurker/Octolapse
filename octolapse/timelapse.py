...
from octolapse.gcode import SnapshotGcodeGenerator, SnapshotGcode
from octolapse.gcode_parser import Commands, ParsedCommand, Response
from octolapse.position import Position
from octolapse.render import RenderError, RenderingProcessor, RenderingCallbackArgs, RenderJobInfo, TimelapseRenderJob
from octolapse.settings import PrinterProfile, OctolapseSettings
from octolapse.snapshot import CaptureSnapshot, SnapshotJobInfo
from octolapse.trigger import Triggers
import octolapse.stabilization_preprocessing as preprocessing
...
class Timelapse(object):
    ...
    def get_position_async(self, start_gcode=None, timeout=None):
        ...
        if not event_is_set:
            ...
            logger.info("Warning:  A timeout occurred while requesting the current position.")
    ...
    def _take_timelapse_snapshot_realtime(
        ...
        try:
            ...
            if not show_real_snapshot_time:
                ...
                logger.info("Stabilization and snapshot process complected in {0} seconds", snapshot_time)
    ...
    def _take_timelapse_snapshot_precalculated(
        ...
        try:
            ...
            if not show_real_snapshot_time:
                ...
                snapshot_time = snapshot_end_time - snapshot_start_time
                logger.info(
                    "Stabilization and snapshot process complected in {0} seconds",
                    snapshot_time
                )
                self.SecondsAddedByOctolapse += snapshot_time
                timelapse_snapshot_payload["current_snapshot_time"] = snapshot_time
    ...
    def on_gcode_queuing(self, command_string, cmd_type, gcode, tags):
        ...
        logger.debug("Queuing Command: gcode:{0}", command_string)
    ...
    def detect_timelapse_start(self, command_string, tags):
        ...
        if (
            ...
            if self._octoprint_printer.set_job_on_hold(True):
                ...
                logger.info(
                    "Print Start Detected.  Command: {0}, Tags:{1}",
                    command_string,
                    tags
                )
        ...
        try:
            ...
            if gcode != "":
                if self._state == TimelapseState.AcquiringLocation:
                    logger.info("Sending triggering command for position acquisition - {0}", gcode)
    ...
    def acquire_snapshot_precalculated(self, parsed_command):
        try:
            logger.info("About to take a snapshot.  Triggering Command: {0}", parsed_command.gcode)
    ...
    def on_gcode_sent(self, cmd, cmd_type, gcode, tags):
        logger.verbose("Sent to printer: {0}", cmd)

    def on_gcode_received(self, line):
        logger.verbose("Received from printer: {0}", line)
        if self._position_request_sent:
    ...
    def _on_prerender_start(self, payload):
        ...
        logger.info("Started prerendering the timelapse. JobId: {0}", payload.JobId)
    ...
    def _on_render_start(self, payload):
        assert (isinstance(payload, RenderingCallbackArgs))
        logger.info("Started rendering/synchronizing the timelapse. JobId: {0}", payload.JobId)
    ...
    def _on_render_success(self, payload):
        assert (isinstance(payload, RenderingCallbackArgs))
        logger.info("Completed rendering. JobId: {0}", payload.JobId)
    ...
    def _on_render_end(self):
        logger.info("Completed all pending rendering jobs.")
