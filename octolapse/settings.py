    ...
    def _update(source, iterable):
        ...
        for key, value in item_to_iterate.items():
            try:
                ...
                logger.exception("Settings._update - Failed to update settings.  Key:{0}, Value:{1}", key, value)
                continue
...
class OctolapseSettings(Settings):
    ...
    def save(self, file_path):
        logger.info("Saving settings to: {0}.", file_path)
        self.save_as_json(file_path)
        logger.info("Settings saved.")

    @classmethod
    def load(cls, file_path, plugin_version):
        logger.info("Loading existing settings file from: {0}.", file_path)
