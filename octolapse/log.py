...
class OctolapseFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record_copy.name.startswith('octolapse.octolapse.'):
            record_copy.name = record_copy.name[10:]
        return super(OctolapseFormatter, self).format(record_copy)
...
class LoggingConfigurator(object):
    ...
    def __init__(self, name=None):
        if name is None:
            raise Exception("The Octolapse logger should be constructed in the root module.  Unable to re-create a "
                            "singleton!")
        self.logging_formatter = OctolapseFormatter()
        self._root_logger = self._get_root_logger(name)
        self._root_logger_name = name
        self._level = logging.DEBUG
        self._file_handler = None
        self._console_handler = None
        self.child_loggers = set()

    def _get_root_logger(self, name=None):
            ...
            if debug_settings is not None:
                for logger_name in self.child_loggers:
                    ...
                    for enabled_logger in debug_settings.enabled_loggers:
                        if enabled_logger["name"] == logger_name:
