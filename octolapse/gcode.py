...
from octolapse.gcode_parser import ParsedCommand
from octolapse.position import Pos, Position
...
class SnapshotGcodeGenerator(object):
    ...
    def get_snapshot_position(self, x_pos, y_pos):
        ...
        if not utility.is_in_bounds(self.BoundingBox, coordinates["X"], None, None):
            ...
            message = "The snapshot X position ({0}) is out of bounds!".format(
                coordinates["X"])
            self.has_snapshot_position_errors = True
            logger.error("gcode.py - GetSnapshotPosition - {0}", message)
    ...
    def return_to_original_position(self):
        if self.return_when_complete:
            ...
            logger.info(
                "Skipping return position, traveling to the triggering command position: X={0}, y={1}",
    ...
    def create_snapshot_gcode(
        ...
        logger.info(
            "Snapshot Gcode - snapshot_commandIndex:{0}, EndIndex:{1}, Triggering Command:{2}",
