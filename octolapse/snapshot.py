...
from octolapse.settings import CameraProfile, OctolapseSettings
import octolapse.utility as utility
from octolapse.gcode_parser import Commands
from tempfile import mkdtemp

from octolapse.utility import TimelapseJobInfo
...
class CaptureSnapshot(object):
    ...
    def clean_snapshots(self, snapshot_directory, job_directory):
        ...
        logger.info("Cleaning snapshots from: {0}", snapshot_directory)
    ...
    def save_image_from_request(self, r):
        ...
        try:
            with i_open(self.snapshot_job_info.full_path, 'wb') as snapshot_file:
                ...
                logger.info("Snapshot - Snapshot saved to disk at {0}", self.snapshot_job_info.full_path)
        except Exception as e:
...
class ExternalScriptSnapshotJob(SnapshotThread):
    ...
    def run(self):
        try:
            logger.info("Snapshot - running {0} script.", self.script_type)
            ...
            logger.info("The {0} script job completed, signaling task queue.", self.script_type)
    ...
    def execute_script(self):
        ...
        if not return_code == 0:
            ...
            logger.error(
                "Error output was returned from the {0} script: {1}",
                self.script_type,
                error_message
            )
        ...
        try:
            ...
            try:
                if len(self.Username) > 0:
                    ...
                    logger.info("Snapshot - downloading from {0:s}.", self.Url)
                    r = requests.get(
