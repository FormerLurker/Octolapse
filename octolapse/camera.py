...
from octolapse.settings import CameraProfile, MjpegStreamerStaticSettings
...
class CameraControl(object):
    ...
    def apply_webcam_settings(cls, camera_profile=None, name=None, address=None, username=None, password=None,
        ...
        if camera_profile != None:
            logger.info("Applying all webcam image settings to the {0} camera.", camera_profile.name)
...
class CameraSettingScriptThread(Thread):
    ...
    def run(self):
        try:
            ...
            logger.info(
                "Executing the 'Before Print Starts' script at {0} for the '{1}' camera.", script, self.camera_name
            )
            if not script:
            ...
            if not return_code == 0:
                ...
                logger.warn(
                    "The console returned an error while running the script at {0} for the '{1}' camera.  Details:{2}",
                    script, self.camera_name, error_message
                )
            else:
                logger.info(
                    "The 'Before Print Starts' script for the {0} camera completed successfully.", self.camera_name
                )
...
class CameraSettingWebRequestThread(Thread):
    ...
    def run(self):
        try:
            logger.debug(
                "Setting {0} to {1} for the '{2}' webcam", self.request["name"], self.request["value"], self.camera_name
            )
            ...
            logger.debug(
                "The {0} setting was applied successfully to the '{1}' camera", self.request["name"], self.camera_name
            )
        except CameraError as e:
            self.error = e
