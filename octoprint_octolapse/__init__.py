	...
	def get_settings_defaults(self):
		return dict(enabled = True,
					...
					profiles= {
						...
						printer = dict(bed_x_max= -1, 
							bed_y_max= -1, 
							bed_z_max= -1,),
						stabilization = dict(x_movement_speed= 0,
							x_type = 'fixed',
							...
							z_movement_speed_mms= 0, # auto
						),
						snapshot = dict(trigger_type = 'layer_change',
							length = 0.2,
							seconds = 30,
							archive = True,
							delay = 1000,
							retract_before_move= False),
						rendering = dict(enabled = True,
