            ...
            for current_trigger in self._triggers:
                ...
                if position.current_pos.has_position_error:
                    self._settings.Logger.log_error(
                        "A trigger has a position error:{0}".format(position.current_pos.position_error))
...
class GcodeTrigger(Trigger):
    ...
    def __init__(self, octolapse_settings):
        ...
        if self.snapshot.extruder_state_requirements_enabled:
            ...
            self._settings.Logger.log_trigger_create(message)
                ...
                if state.is_waiting:
                    if position.is_extruder_triggered(self.extruder_triggers):
                        if self.require_zhop and not position.current_pos.is_zhop:
                            ...
                            self._settings.Logger.log_triggering(
            ...
            if not position.current_pos.has_homed_position:
                ...
                if (
                    ...
                    if new_increment <= state.current_increment:
                        ...
                        ).format(self.height_increment, state.current_increment, new_increment)
                        self._settings.Logger.log_trigger_height_change(message)
                    else:
                        ...
                        ).format(self.height_increment, state.current_increment, position.current_pos.height)
                        self._settings.Logger.log_trigger_height_change(message)
    ...
    def resume(self):
        try:
            ...
            if state.pause_time is not None and state.trigger_start_time is not None:
                ...
                self._settings.Logger.log_timer_trigger_unpaused(message)
    ...
    def update(self, position):
        try:
            ...
            if not position.current_pos.has_homed_position:
                ...
                self._settings.Logger.log_trigger_time_remaining(message)
                ...
                if state.seconds_to_trigger <= 0:
                    ...
                    if position.is_extruder_triggered(self.extruder_triggers):
                        if self.require_zhop and not position.current_pos.is_zhop:
                            ...
                            state.is_waiting_on_feature = False
                            # Log trigger
                            self._settings.Logger.log_triggering('TimerTrigger - Triggering.')
